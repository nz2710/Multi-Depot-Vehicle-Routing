...
class KmeanMVRP:
    ...
    def kmean_assign_depots_closest_centroids(self, X, De, num_depots, num_clusters, max_iter=300):
        # Phân cụm các customers
        # kmeans = KMeans(n_clusters=num_clusters, random_state=11).fit(X)
        # labels = kmeans.predict(X)
        # centroids = kmeans.cluster_centers_
        (centroids, labels, _) = self.kmean_core.kmeans(X, num_clusters, max_iter)
        ...
        list_depot_index = list(range(num_depots))
        # random.shuffle(list_depot_index)
        for depot_index in list_depot_index:
