...
def Clark_Wright_Savings_Solver(X_pairwise,
    ...
    node_list_origin = []
    for node in depots_customers_metadata[depot_index][cluster_index]:
        node_list_origin.append(node)
    ...
    for node in node_list_origin:
        if node not in node_not_assigneds:
            node_assigneds.append(node)
    if len(node_not_assigneds) > 0:
        print("Error: Có một số customers không thoả mãn ràng buộc max_duration_route hoặc max_vehicles !!!")
        return [], {}, node_assigneds, node_not_assigneds

    return routes, routes_metada_depot, node_assigneds, node_not_assigneds